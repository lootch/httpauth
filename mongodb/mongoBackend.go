...
func (b MongodbAuthBackend) User(username string) (user httpauth.UserData, e error) {
	var result httpauth.UserData
	...
	if err != nil {
		return result, httpauth.ErrMissingUser
	}
	return result, nil
}

// Users returns a slice of all users.
func (b MongodbAuthBackend) Users() (us []httpauth.UserData, e error) {
	c := b.connect()
	defer c.Database.Session.Close()
...
func (b MongodbAuthBackend) SaveUser(user httpauth.UserData) error {
	c := b.connect()
	defer c.Database.Session.Close()
