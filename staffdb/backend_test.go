...
func testBackendAuthorizer(t *testing.T, backend httpauth.AuthBackend) {
	_, err := httpauth.NewAuthorizer(backend, []byte("testkey"), "user", roles)
	if err != nil {
		t.Fatal(err)
	}
}

func testBackendSaveUser(t *testing.T, backend httpauth.AuthBackend) {
	user2 := httpauth.UserData{"username2", "email2", []byte("passwordhash2"), "role2"}
	if err := backend.SaveUser(user2); err != nil {
		t.Fatalf("SaveUser sql error: %v", err)
	}

	user := httpauth.UserData{"username", "email", []byte("passwordhash"), "role"}
	if err := backend.SaveUser(user); err != nil {
		t.Fatalf("SaveUser sql error: %v", err)
	}
}

func testBackendNewAuthBackend_existing(t *testing.T, backend httpauth.AuthBackend) {
...
func testBackendUser_existing(t *testing.T, backend httpauth.AuthBackend) {
...
func testBackendUser_notexisting(t *testing.T, backend httpauth.AuthBackend) {
	if _, err := backend.User("notexist"); err != httpauth.ErrMissingUser {
		t.Fatal("Not existing user found.")
	}
}

func testBackendUsers(t *testing.T, backend httpauth.AuthBackend) {
	var (
		u1 httpauth.UserData
		u2 httpauth.UserData
	)
...
func testBackendUpdateUser(t *testing.T, backend httpauth.AuthBackend) {
	user2 := httpauth.UserData{"username", "newemail", []byte("newpassword"), "newrole"}
	if err := backend.SaveUser(user2); err != nil {
		t.Fatalf("SaveUser sql error: %v", err)
	}
...
func testBackendDeleteUser(t *testing.T, backend httpauth.AuthBackend) {
	...
	} else if err != httpauth.ErrDeleteNull {
		t.Fatalf("DeleteUser raised unexpected error: %v", err)
	}
}

func testBackendClose(t *testing.T, backend httpauth.AuthBackend) {
	backend.Close()
}

func testBackend(t *testing.T, backend httpauth.AuthBackend) {
...
func testAfterReopen(t *testing.T, backend httpauth.AuthBackend) {
	users, err := backend.Users()
	if err != nil {
		t.Fatal(err.Error())
	}
...
func testDelete2(t *testing.T, backend httpauth.AuthBackend) {
	if err := backend.DeleteUser("username2"); err != nil {
		t.Fatalf("DeleteUser error: %v", err)
	}
}

func testClose2(t *testing.T, backend httpauth.AuthBackend) {
	backend.Close()
}

func testBackend2(t *testing.T, backend httpauth.AuthBackend) {
