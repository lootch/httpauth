...
import (
	...
	"github.com/lootch/httpauth/sqldb"
	_ "github.com/lib/pq"
)

var (
	backend     sqldb.SqlAuthBackend
...
func main() {
	...
	// create the backend
	connStr := "postgres://admin:private@rattle/auth?sslmode=disable"
	backend, err = sqldb.NewSqlAuthBackend("postgres", connStr)
	if err != nil {
		panic(err)
	}
...
func getLogin(rw http.ResponseWriter, req *http.Request) {
	switch method := req.Method; strings.ToLower(method) {
	case "get":
